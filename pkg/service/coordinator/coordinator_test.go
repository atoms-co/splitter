...
func TestCoordinator_SingleConsumer(t *testing.T) {
	synctest.Run(func() {
		...
		elements := assertx.Drain(out)
		assert.False(t, slices.ContainsFunc(elements, func(msg model.ConsumerMessage) bool { _, ok := isAssign(msg); return ok }), "consumer should not receive assign messages while draining")
...
func TestCoordinator_ConsumerReconnectsWhileSuspended(t *testing.T) {
	synctest.Run(func() {
		...
		for _, msg := range assertx.Drain(out2) {
			_, ok := isExtend(msg)
			assert.True(t, ok, "suspended consumer should only receive extend messages")
		}
...
func TestCoordinator_ConsumerSuspendAndResume(t *testing.T) {
	synctest.Run(func() {
		...
		for _, msg := range assertx.Drain(out) {
			_, ok := isExtend(msg)
			assert.True(t, ok, "suspended consumer should only receive extend messages")
		}
