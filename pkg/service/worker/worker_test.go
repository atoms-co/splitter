...
func TestWorker(t *testing.T) {
	...
	t.Run("consumer/connect", func(t *testing.T) {
		synctest.Test(t, func(t *testing.T) {
			...
			in := chanx.NewFixed(model.NewRegister(consumer, service2, nil, nil, model.NewOptions(), model.NewConsumerMetadata("")))

			_, err := d.worker.Connect(d.ctx, session.NewID(), location.NewInstance(location.New("centralus", "splitter1")), in)
			require.NoError(t, err)

			in2 := chanx.NewFixed(model.NewRegister(consumer, service1, nil, nil, model.NewOptions(), model.NewConsumerMetadata("")))
