...
func NewDispatcher(ctx context.Context, cl clock.Clock, client ConsumerClient, loc location.Location, endpoint string, service QualifiedServiceName, chain []DispatchFilter, opts ...ConsumerOption) *Dispatcher {
	...
	clusters, closed := client.Join(wctx, ret.id, ret.service, ret.handle, opts...) // closed == all grants are relinquished
...
// Processor manages Ranges for a domain on single node, acting as a Proxy. A Range is created for
...
func NewProcessor[T, K any, V Range](cl clock.Clock, domain DomainName, rfn RemoteFn[T], fn func(K) DomainKey, factory RangeFactory[V]) *Processor[T, K, V] {
	return &Processor[T, K, V]{
		cl:          cl,
		domain:      QualifiedDomainName{Domain: domain},
		rfn:         rfn,
		fn:          fn,
		factory:     factory,
...
func (p *Processor[T, K, V]) TryHandle(ctx context.Context, grant GrantID, shard Shard, lease Ownership) bool {
	...
	if shard.Domain != p.domain {
		return false // skip: not handling domain
	}

	p.handle(ctx, grant, shard, lease)
	return true
}

func (p *Processor[T, K, V]) handle(ctx context.Context, grant GrantID, shard Shard, lease Ownership) {
...
func (p *Processor[T, K, V]) Lookup(key K) (V, bool) {
	<-p.initialized.Closed()
	return p.grants.Lookup(QualifiedDomainKey{Domain: p.domain, Key: p.fn(key)})
}
...
func (p *Processor[T, K, V]) key(key K) QualifiedDomainKey {
	return QualifiedDomainKey{Domain: p.domain, Key: p.fn(key)}
}
...
func (p *ProxyStub[T, K, V]) Lookup(key K) (V, bool) {
	<-p.initialized.Closed()
	return p.proxy.Lookup(key)
}
